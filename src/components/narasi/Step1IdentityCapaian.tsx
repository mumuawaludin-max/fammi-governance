...
import type { Jenjang, ICapaianFormState, NarrativeTone } from "@/types/narasi";
import { cn } from "@/lib/cn";

interface Step1IdentityCapaianProps {
...
export function Step1IdentityCapaian({ state, onChange, onNext }: Step1IdentityCapaianProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-base font-bold text-text-primary">Identitas Sekolah, Jenjang &amp; Nada Narasi</h2>
        <p className="text-xs text-text-secondary mt-1">
          Informasi ini digunakan untuk menyesuaikan tone narasi sesuai usia perkembangan anak dan gaya bahasa yang dipilih.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-2">
        <label className="text-xs font-semibold text-text-primary">Nada Narasi</label>
        <p className="text-[10px] text-text-secondary -mt-1">
          Pilih gaya bahasa narasi yang ingin digunakan dalam preview dan export.
        </p>
        <div className="flex gap-3">
          {(["islami", "general"] as NarrativeTone[]).map((tone) => {
            ...
            const desc =
              tone === "islami"
                ? "Menggunakan gaya bahasa hangat dengan nuansa Islami seperti Alhamdulillah, insyaallah, dan doa penutup secara proporsional."
                : "Menggunakan gaya bahasa hangat, netral, dan inklusif tanpa frasa religius, namun tetap memakai sapaan khas Fammi: Ayah Bunda dan ananda.";
            return (
              <button
                key={tone}
                type="button"
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className={cn("text-xs font-semibold", isSelected ? "text-fammi" : "text-text-primary")}>
                    {label}
                  </span>
                </div>
